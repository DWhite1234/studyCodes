 ...
 * 窗口允许延迟数据,迟到数据和侧输出流
 */
public class flink_forBoundedOutOfOrderness_allowedLateness_sideOutputLateData {
    public static void main(String[] args) throws Exception {
        ...
        SingleOutputStreamOperator<WaterSensor> result = waterSensorSource.assignTimestampsAndWatermarks(waterSensorWatermarkStrategy)
                .keyBy("id")
                .window(TumblingEventTimeWindows.of(Time.seconds(5)))
                .allowedLateness(Time.seconds(3))
                .sideOutputLateData(new OutputTag<WaterSensor>("slide"){})
                .sum("vc");

        result.print();
        result.getSideOutput(new OutputTag<WaterSensor>("slide") {
        }).print("silde");
